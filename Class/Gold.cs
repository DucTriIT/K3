...
public class Gold
{
   ...
   {        
        ...
        if (_temp5.Length == 3)
        {
            _temp3 = _strReturn.Substring(_strReturn.IndexOf(" ") + 1);
            _strReturn = _strReturn.Substring(0, _strReturn.IndexOf(" ")) + " Chỉ" + _temp3.Substring(_temp3.IndexOf(" "));
        }
        ...
        if (_temp5.Length == 2)
        {
            if (_temp5 == "10")
                _strReturn = "Một Phân ";
            else if (decimal.Parse(_temp5) <= 19)
            {
                _strReturn = "Một phân " + _strReturn.Substring(_strReturn.IndexOf(" "));
            }
            else
            {
                _temp3 = _strReturn.Substring(_strReturn.IndexOf(" ") + 1);
                _strReturn = _strReturn.Substring(0, _strReturn.IndexOf(" ")) + " phân" + _temp3.Substring(_temp3.IndexOf(" "));
            }
        }
        if (_temp5.Length == 1 && _temp5 != "0")
        {            
            _strReturn = _strReturn + " ly ";
        }
        #endregion
        ...
        if (__strReturn2.Length > 0)
        {
            if(_strReturn.Length >0)
                return _strReturn + "Chấm " + __strReturn2 + " Zen";
            return "Không Chấm " + __strReturn2 + " Zen";
        }
        return _strReturn;
    }
