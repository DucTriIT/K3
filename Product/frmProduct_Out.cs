...
namespace GoldRT
{
    public partial class frmProduct_Out : DevExpress.XtraEditors.XtraForm
    {
    ...
    #region Public Functions
        public frmProduct_Out()
        {
            InitializeComponent();
        }

        public frmProduct_Out(string _TrnID)
        {
            InitializeComponent();
            StrTrnID = _TrnID;
        }
    ...
    #region Private Functions
            ...
            try
            {
                ...
                if (strID == "")
                {
                    ds = clsCommon.ExecuteDatasetSP("[TRN_PRODUCT_OUT_Ins]", "", DateTime.Now.ToString("dd/MM/yyyy"),
                        ...
                        strGroupID,txtPriceUnit.Text=="L"? (decimal.Parse(txtTotalWeight.Text)*hs).ToString():decimal.Parse(txtTotalWeight.Text).ToString(),
                        txtPriceUnit.Text == "L" ? (decimal.Parse(txtDiamondWeigth.Text) * hs).ToString() : decimal.Parse(txtDiamondWeigth.Text).ToString(), txtPriceUnit.Text == "L" ? (decimal.Parse(strStampWeight) * hs).ToString() : decimal.Parse(strStampWeight).ToString(),
                        decimal.Parse(txtTaskPrice.Text).ToString(), txtReason.Text.Trim(), clsSystem.UserID, clsSystem.UserID, "W",txtGiaBanMon.EditValue,txtSL.EditValue);
                }
                else
                {
                    ds = clsCommon.ExecuteDatasetSP("[TRN_PRODUCT_OUT_Upd]", strID, null, null, strProductID,
                        txtProductCode.Text.Trim(), strGroupID,
                       txtPriceUnit.Text=="L"? (decimal.Parse(txtTotalWeight.Text)*hs).ToString():decimal.Parse(txtTotalWeight.Text).ToString(),
                        txtPriceUnit.Text == "L" ? (decimal.Parse(txtDiamondWeigth.Text) * hs).ToString() : decimal.Parse(txtDiamondWeigth.Text).ToString(), txtPriceUnit.Text == "L" ? (decimal.Parse(strStampWeight) * hs).ToString() : decimal.Parse(strStampWeight).ToString(), decimal.Parse(txtTaskPrice.Text).ToString(), txtReason.Text.Trim(),
                        clsSystem.UserID, clsSystem.UserID, "W", txtGiaBanMon.EditValue,txtSL.EditValue);
                }
        ...
        private void rtxtSL_EditValueChanged(object sender, EventArgs e)
        {
            
        }

     
        //private void cboProductCode_Validating(object sender, CancelEventArgs e)
        //{
        //    string m_strProductCode = txtProductCode.Text.Trim();
        //    if (m_strProductCode == "") return;
